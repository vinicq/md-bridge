# PDF/Word to Markdown reference checklist

Reference material for the `pdf-to-markdown` package. Use it after running `convert.py` to verify conversion quality and decide what to edit manually.

The canonical Markdown dialect the converters target (CommonMark 0.31.2 plus a named GFM and Pandoc subset) is declared in [ADR-001](../../docs/architecture/adr-001-markdown-dialect.md).

## What Markdown can and cannot represent

| Category | Examples in Word/PDF | Mappable to Markdown? |
|---|---|---|
| Headings | Title, Heading 1-6 | Yes (`#` to `######`) |
| Paragraphs | Body text, breaks | Yes |
| Basic emphasis | Bold, italic, both | Yes (`**`, `*`, `***`) |
| Other font styles | Underline, small caps, strikethrough | Strikethrough emitted as GFM `~~`; underline and small caps drop to plain text |
| Font family, size, color | Arial 12pt red | No (CSS/theme territory) |
| Highlight annotation | Highlighter pen over text | Opt-in: `--extract-highlights` emits `==text==` (rendered as `<mark>`) |
| Alignment | Left, center, right, justify | Partial (HTML or GFM tables only) |
| Spacing, indent | Line spacing, first-line indent, tabs | No semantic |
| Lists | Bullets, numbered, multi-level | Yes; `--tight-loose-lists` keeps tight/loose spacing (#168), `--nested-ordered-lists` keeps a nested ordered sublist's own start and indent (#194) |
| Tables | Headers, cells, merged cells | Yes for simple; `--table-column-align` emits GFM alignment (#175); `--multiline-table-format=grid` emits a Pandoc grid table so a multi-line cell survives (#166, needs the `grid-tables` extra to render back) |
| Definition lists | Glossary term + indented definition | Opt-in: `--detect-definition-lists` emits `Term` / `: definition` (rendered as `<dl>`) for a run of at least two pairs (#161) |
| Images | Inline images | Yes (`![]()`); `--image-width-hints` emits `{width=N}` (#169); `--image-link-anchors` keeps a click-through link as `[![alt](src)](target)` (#170) |
| Shapes, SmartArt | Drawings, diagrams | No (rasterize as image) |
| Text boxes | Floating boxes | Partial |
| Columns | Multi-column layout | No |
| Page breaks | Section/page breaks | Partial (`---`) |
| Headers/footers | Page numbers, doc title | No (filter out) |
| Hyperlinks | URLs, emails, anchors | Yes (`[text](url)`) |
| Footnotes/endnotes | Numbered footnotes | Yes via extensions (`[^1]`) |
| TOC | Auto-generated index | Partial (regenerated by renderer) |
| Cross-references | "see Figure 3" | Partial: `--emit-figure-anchors` gives a numbered figure a `{#fig-N}` target; the reference text is not auto-linked |
| Comments | Review comments | No (or HTML `<!-- -->`) |
| Track changes | Insertions/deletions | No native |
| Styles, themes | Quote style, theme set | Partial (Quote becomes `>`) |
| Metadata | Author, title, date | YAML front matter |

## Post-conversion checklist

After running `convert.py <pdf> -o <md>`, open the `.md` and walk through this list. Treat each `[ ]` as "verify or edit".

```markdown
# Checklist for PDF/Word to Markdown

## 1. Document structure
- [ ] One `#` H1 (the document title). Fix any cover-page splits like "Foundation Level" plus "Syllabus" into "Foundation Level Syllabus" if needed.
- [ ] Heading hierarchy is logical: `##` then `###`, no jumps from `##` to `####`.
- [ ] Paragraphs separated by blank lines, no random hard breaks mid-sentence.
- [ ] Stray page numbers and `---` separators removed where not wanted.

## 2. Headings and TOC
- [ ] No body paragraph accidentally promoted to heading because of font size noise.
- [ ] No HTML tags (`<h1>`, `<b>`) left over.
- [ ] Doc's own TOC page (if present): kept as plain text, or replaced with renderer-generated TOC, or converted to a bulleted list of `[Title](#anchor)` links.

## 3. Inline emphasis
- [ ] `**bold**` and `*italic*` correct; no `**texto **` or `__*texto*__` artifacts.
- [ ] Underlines (decorative-only) ignored or reinterpreted.
- [ ] Strikethrough (`~~text~~`) only where it carried meaning.
- [ ] Superscript readable as Pandoc caret: `x^2^` (raw `<sup>` no longer emitted).
- [ ] No spurious whitespace inside emphasis markers.

## 4. Lists
- [ ] `- ` (or `* `) used consistently for unordered lists.
- [ ] Numbered lists keep correct order (1, 2, 3...).
- [ ] Multi-level lists have consistent indentation and nest visually.
- [ ] Task lists (`- [ ]`, `- [x]`) only where the original used checkboxes.
- [ ] No false bullets from leftover dashes/brackets.

## 5. Tables
- [ ] Each table has a header row + separator (`| --- | --- |`).
- [ ] Pipe count consistent across all rows.
- [ ] Merged cells revised or split into two tables.
- [ ] Column alignment markers (`:---`, `:---:`, `---:`) added where it matters.
- [ ] Tables wrongly extracted as plain text are manually rebuilt.

## 6. Images
- [ ] Each relevant image present as `![alt](path)` in reading order.
- [ ] Alt text descriptive, not `image1.png`.
- [ ] Captions kept near the image.
- [ ] Rasterized shapes/SmartArt readable; if not, annotate with a text description.

## 7. Links and notes
- [ ] All hyperlinks as `[text](url)`, not bare URLs (unless that's the project style).
- [ ] Internal cross-references work or replaced with section references.
- [ ] Footnotes converted to `[^n]` syntax or organized at the end.
- [ ] "See page 12" style references rewritten to "see section X" (Markdown has no pages).

## 8. Code and quotes
- [x] Monospaced excerpts as `` `inline` `` or fenced blocks with language tag.
- [ ] Block quotes (`> `) restored where the original used Quote style.
- [ ] Formulas legible: use LaTeX inline (`$x^2$`) or block (`$$ ... $$`) if needed.

## 9. PDF/Word noise removal
- [ ] Repeated headers/footers across pages stripped.
- [ ] Extra spaces/tabs/hard breaks cleaned.
- [ ] Lines broken mid-sentence joined into full paragraphs.
- [ ] OCR artifacts (`ﬁ`, `ﬂ`, weird Unicode bullets) reviewed.

## 10. Metadata / front matter
- [ ] YAML front matter at top with `title`, `author`, `date`, `source` (when applicable).
- [ ] Cover-page metadata (for example "Relatório Técnico - Empresa X - 2025") promoted to front matter, not left in the body.
- [ ] Converter banners ("Converted by Tool X...") removed.

## 11. Style consistency
- [ ] Single bullet marker (`-` OR `*`) used throughout.
- [ ] One blank line before/after every heading.
- [ ] Table style consistent (alignment, spacing).
- [ ] If a formatter is used (Prettier etc.), tables and code blocks survive intact.
```

## What `convert.py` already handles automatically

The current converter (`scripts/convert.py`) already covers items 1, 2, 3 (most), 4, 5, 6, 7 (hyperlinks), 8 (monospace-flagged spans wrapped as `` `inline` `` and blocks ≥70% mono fenced with language inference for python/javascript/sql/html/json), 9 (header/footer filtering, paragraph merge, dot-leader stripping), 10 (front matter from PDF metadata). The remaining parts of 8 (formulas) and the more nuanced parts of 11 require manual review per document.

Block quotes are detected **opt-in**: pass `--detect-blockquotes` to `convert.py` (or `{"detect_blockquotes": true}` in the API options). A body-size block inset from the margin on every line becomes a `>` quote. It ships off by default because a list continuation is also an indented body block; when the flag is on, an indented block under an open list still binds to the item as content, not a quote.

Heading levels beyond H3 are **opt-in** and gated on clustering: pass `--cluster-headings` (`{"cluster_headings": true}`) to map font sizes to heading levels by gap-partitioned size bands, and `--max-heading-level N` (`{"max_heading_level": N}`, 1-6) to allow H4-H6. `max_heading_level` defaults to 3 and only affects output when `cluster_headings` is on; the default fixed-cutoff path still tops out at H3, so default output is unchanged. A flat size histogram yields fewer levels naturally; levels with no font-size evidence are never synthesized.

Footnote pairing is **opt-in** (#148): pass `--footnote-pairing` (`{"footnote_pairing": true}`) to lift small-font bottom-of-page `N text` blocks out of the body and re-emit them as a sorted GFM `[^N]: text` tail, while a body superscript digit `N` whose number has a matching definition becomes a `[^N]` reference. A pre-scan over the whole document collects the definition numbers before the page loop, so a reference on one page still rewrites against a definition on another. It ships off by default because a numbered list item at the bottom of a page can look like a definition; with the flag off the output stays byte-identical. First definition wins on a number collision.

Heading anchors are **opt-in** (#152): pass `--emit-heading-anchors` (`{"emit_heading_anchors": true}`) to append a deterministic `{#slug}` to each heading, in the Pandoc/mkdocs attribute dialect (`## Title {#slug}`). The slug lowercases the text, ASCII-folds accents (NFKD, drop combining marks), collapses non-alphanumeric runs to a single `-`, strips framing `-`, and caps at 64 characters; an empty result falls back to `section`. Slugs are deduplicated document-wide in emission order (`summary`, `summary-2`, `summary-3`). A document-level post-pass over the assembled body does the dedup across pages, skips `#` lines inside code fences, and leaves a heading that already carries `{#...}` untouched. It ships off by default so output stays byte-identical; a renderer that does not parse the attribute syntax shows the slug as literal trailing text (acceptable degradation).

Abbreviation glossaries are **opt-in** (#163): pass `--extract-abbreviations` (`{"extract_abbreviations": true}`) to detect a "List of Abbreviations / Acronyms / Siglas / Abreviaturas" section (locale-aware heading match, en/pt/es) and emit a `*[TOKEN]: expansion` block at the document tail. The markdown-to-pdf renderer loads the `abbr` extension, so each later occurrence of `TOKEN` in the body renders as `<abbr title="expansion">`. A pre-scan reads the glossary page after the heading and pairs the two real layouts: side-by-side columns (token left, expansion right, same baseline) and one line per entry with the token leading (`AR Assertion Roulette`). The layout mode is fixed by the first page so a body page after the table cannot leak rows; a token must be a single mostly-uppercase run of at most ten characters, and a section needs at least two valid pairs, so a table-of-contents entry like "Lista de Siglas" is skipped in favour of the real glossary. A "Glossary" heading (a prose definition list) and an ISO-style numbered "3 Terms, definitions and abbreviations" clause never match, by design. It ships off by default so output stays byte-identical; the raw glossary stays in the body untouched (the feature is additive). Known limits: the heading and its rows must share a page, and a glossary mixed with body text on one page is not detected.

Smart typography is **opt-in** (#171): a document-level post-pass folds Unicode typography back to ASCII. Pass `--smart-typography-quotes ascii` (`{"smart_typography_quotes": "ascii"}`) to map curly quotes to straight, `--smart-typography-ellipsis ascii` to map `…` to `...`, and `--smart-typography-dashes ascii` to map en/em dash to `--`/`---`. Each defaults to `preserve`, so the default output keeps the source glyphs and stays byte-identical. The pass protects code (fenced, indented, inline), images, links, autolinks, and reference-definition lines, so a quote or dash inside a URL or a code sample is never rewritten; it only ever removes a Unicode source character, so it is idempotent. Out of scope: straight-to-curly synthesis (the renderer's `smarty` does that), and NBSP form/preservation (the converter normalizes a non-breaking space to a regular space upstream, so there is none left to reshape; preserving it needs a separate upstream change).

Image caption alt text is **opt-in** (#149): with `--with-images`, pass `--caption-alt-text` (`{"caption_alt_text": true}`) to use a small-font line within 24 pt below an image as that image's alt text, emitting `![caption](path)` instead of `![](path)` and dropping the caption line from the body so it is not duplicated. The caption must be small-font (a body-size paragraph is never claimed), each caption is used by at most one image, and an image with no caption below it keeps an empty alt. The alt text is sanitized: whitespace collapses, surrounding quotes are stripped, and `]` is escaped. Off by default, and only relevant under `--with-images` (the default path extracts no images), so output is otherwise unchanged. Fills the empty-alt gap that fails WCAG 1.1.1 for captioned figures.

## HTML emission policy (#154)

The converter emits pure Markdown by default and never writes raw HTML tags: superscript becomes Pandoc `^x^`, strikethrough GFM `~~text~~`, small-font captions plain text. A grep of any output for `<tag>` returns nothing.

Raw HTML is opt-in and capped. The `allow_html` option takes a set of tag names; only tags both requested and inside the hard cap (`sup`, `sub`, `small`, `kbd`, `abbr`) are ever emitted. Anything else (`script`, `style`, `iframe`, `a`, `img`, ...) is rejected at the API with `422 invalid_options`. Every potential HTML emission routes through one helper, `emit_html(tag, inner, allow_html)`, which drops a disallowed tag and logs a warning. Today no conversion path emits HTML even with `allow_html` set; the option reserves the contract and the single chokepoint for a future feature that needs a tag with no Markdown equivalent.

## What this package does NOT do

- **OCR** for scanned PDFs. Run Tesseract with `por`/`eng` first; check `inspect_pdf.py` output. Zero fonts means the PDF is scanned.
- **Math formula recognition**: PyMuPDF returns formula characters as plain text. Reformulate manually if precision matters.
- **Footnote correlation** is **opt-in** (#148): by default small text at the page bottom is rendered inline as plain text (the `<small>` wrapper was dropped in #141). Manual rework if needed.
- **Merged-cell tables**: extracted as best-effort flat tables. Complex cases need manual restructuring.
- **Multi-column layouts**: blocks come in PyMuPDF's reading order, which can interleave for academic two-column papers.
